...
# exp.Query covers: Select, Union, Except, Intersect, Subquery
# exp.Describe covers: DESCRIBE TABLE, DESC, DESCRIBE EXTENDED
_ALLOWED_TYPES = (exp.Query, exp.Describe)

# Command.this keywords that are safe (sqlglot parses SHOW/EXPLAIN as Command fallbacks)
_ALLOWED_COMMAND_KEYWORDS: frozenset[str] = frozenset({"SHOW", "EXPLAIN"})


def validate_read_only(query: str, dialect: str = "databricks") -> None:
    """Validate that a SQL string contains exactly one read-only statement.

    Raises ReadOnlyViolation if not read-only.
    Raises ParseError on unparseable SQL (fail-closed — query is not executed).
    """
